...
if __name__ == '__main__':
    ...
    print(f"[{args.nn}] [{'FAKE' if args.columns else 'REAL'} {args.dataset}] [{alpha}] [{args.columns if args.columns else dataset.num_features}]   Mean: {performance.mean():.6f} s   |   Std: {performance.std():.6f} s   |   Min: {performance.min():.6f} s   |   Max: {performance.max():.6f} s")